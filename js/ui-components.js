...
async function showCodePreview(username, challengeId) {
    ...
    const isAiReviewCompleted = submission.aiReviewStatus === 'completed' && !!submission.aiReview;

    // Challenge details shown above both submission and AI review
    const challengeTopSection = document.createElement('div');
    challengeTopSection.style.cssText = `
        padding: 16px 20px;
        border-bottom: 1px solid var(--border-color);
        background: var(--bg-primary);
        overflow-y: auto;
    ...
    if (challengeContext) {
        ...
        titleDiv.style.cssText = 'font-weight: bold; margin-bottom: 8px; font-size: 14px; color: var(--text-primary);';
        titleDiv.textContent = `📌 ${challengeContext.title || challengeId}`;
        challengeTopSection.appendChild(titleDiv);
        ...
        if (challengeContext.problem) {
            ...
            challengeTopSection.appendChild(problemLabel);

            const problemBox = document.createElement('div');
            problemBox.style.cssText = 'background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: 4px; padding: 10px; white-space: pre-wrap; line-height: 1.5; margin-bottom: 10px; color: var(--text-primary);';
            problemBox.textContent = challengeContext.problem;
            challengeTopSection.appendChild(problemBox);
        }
        ...
        if (challengeContext.expectedOutput) {
            ...
            challengeTopSection.appendChild(outputLabel);

            const outputBox = document.createElement('div');
            outputBox.style.cssText = 'background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: 4px; padding: 10px; white-space: pre-wrap; line-height: 1.5; margin-bottom: 10px; color: var(--text-primary);';
            outputBox.textContent = challengeContext.expectedOutput;
            challengeTopSection.appendChild(outputBox);
        }
        ...
        if (isAiReviewCompleted && challengeContext.solution) {
            ...
            hiddenNote.style.cssText = 'background: #fff8e1; border: 1px solid #ffecb3; border-radius: 4px; padding: 10px; color: #8a6d3b; font-size: 12px;';
            hiddenNote.textContent = isAiReviewCompleted
                ? 'No reference solution available for this challenge.'
                : 'Reference solution will be shown after AI review is completed.';
            challengeTopSection.appendChild(hiddenNote);
        }
    } else {
        challengeTopSection.innerHTML = `<div style="font-size: 13px; color: var(--text-primary);">📌 Challenge details could not be loaded for <strong>${challengeId}</strong>.</div>`;
    }
    ...
    reviewContainer.style.cssText = `
        ...
        overflow-x: hidden;
        padding: 20px;
        background: ${isDarkModeReview ? 'linear-gradient(135deg, rgba(213, 227, 57, 0.15) 0%, rgba(213, 227, 57, 0.08) 100%)' : 'linear-gradient(135deg, rgba(18, 21, 28, 0.08) 0%, rgba(213, 227, 57, 0.12) 100%)'};
        display: flex;
        flex-direction: column;
        min-width: 0;
    ...
    modalContent.appendChild(challengeTopSection);
